...
export type CreateNftSwapOfferRequestParams = {
...
export type CreateNftSwapOfferRequestProps = {
  request: CreateNftSwapOfferRequestParams;
  popupId: number | undefined;
  onResponse: () => void;
};

export const CreateNftSwapOfferRequest = ({ request, popupId, onResponse }: CreateNftSwapOfferRequestProps) => {
  useSignals();
  const { theme } = useTheme();
  const { addSnackbar } = useSnackbar();
  const { isPasswordRequired } = useWeb3Context();
  const { syncTokens, createNftSwapOffer, isProcessing, setIsProcessing } = useRadiantTokens();
  ...
  const cancel = () => {
    ...
    chrome.runtime.sendMessage({ action: 'createNftSwapOfferResponse', cancelled: true });
    storage.remove('createNftSwapOfferRequest');
  ...
  const handleConfirm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await createNftSwapOffer({
    ...
    if ('error' in result) {
      const msg =
        ...
        result.error === 'insufficient-funds'    ? 'NFT not found in wallet.'     :
        result.error === 'offer-token-not-found' ? `NFT not found in wallet. Sync first.` :
        `Error: ${result.error}`;
      chrome.runtime.sendMessage({ action: 'createNftSwapOfferResponse', error: result.error });
      addSnackbar(msg, 'error');
      return;
    }

    responseSent.current = true;
    addSnackbar('NFT offer signed!', 'success');
    chrome.runtime.sendMessage({ action: 'createNftSwapOfferResponse', partialRawtx: result.partialRawtx });
  ...
  return (
    <>
      <Show when={isProcessing}>
        <PageLoader theme={theme} message="Creating NFT offer…" />
      </Show>
      <Show when={!isProcessing}>
        <ConfirmContent>
          <BackButton onClick={cancel} />
          <HeaderText theme={theme}>List NFT for Trade</HeaderText>
          ...
          <div style={{ width: '85%', background: theme.mainBackground, borderRadius: 8, padding: '12px 14px', margin: '8px 0 4px' }}>
            <Text theme={theme} style={{ margin: '4px 0', fontSize: '0.85rem' }}>
              You offer: <strong>{request.offerTokenName ?? 'NFT'}</strong>
            </Text>
            <Text theme={theme} style={{ margin: '4px 0', fontSize: '0.85rem' }}>
              You want: <strong>{wantDisplay}</strong>
            </Text>
          </div>

          <Text theme={theme} style={{ fontSize: '0.7rem', color: theme.gray, margin: '6px 0 10px', textAlign: 'center', lineHeight: 1.5 }}>
            Your NFT will be locked in the listing until a buyer completes the trade atomically.
          </Text>
          ...
          <FormContainer noValidate onSubmit={handleConfirm}>
            ...
            <Button theme={theme} type="primary" label="Sign & List NFT" isSubmit disabled={isProcessing} />
